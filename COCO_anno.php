...
$query_box = "SELECT box_id, x_min, x_max, y_min, y_max, category ".
             "FROM box WHERE img_id='".$imgID.".jpg';";
$result_box = $conn->query($query_box);
$boxCoordDict = array();
$boxCategoryDict = array();
if($result_box->num_rows > 0) {
    while($row = $result_box->fetch_assoc()) {
        ...
        $boxCategoryDict[$boxID] =  $row["category"];
    }
}
...
$query_img = "SELECT width, height, anno_comments, img_url ".
...
/**Returns the string representation of the
 * box-category dictionary (used for javascript)
 *
 * @return string - boxCategoryDict
 */
function getBoxCategoryDictStr()
{
    global $boxCategoryDict;
    $boxArrStr = "";
    foreach($boxCategoryDict as $boxID => $category)
        $boxArrStr .= $boxID."#".$category."|";
    return $boxArrStr;
}
...
/**Executes an array of update queries using the global conn
 *
 * @param $queries array The queries to execute
 */
function runUpdateQueries(array $queries)
{
    global $conn, $debugStr;
    ...
    if(sizeof($queries) > 0) {
        ...
        for($i=0; $i<sizeof($queries); $i++) {
            $debugStr.=$queries[$i];
            $debugStr.="\n";
            if($conn->query($queries[$i]) === FALSE) {
                die('Failed to update DB with <br/>'.$queries[$i]);
            }
        }
    }
}
...
function updateTables()
{
    ...
    //Delete everything out of the chain / chunk / mention
    //table for this image. This feels dangerous, but it's
    //necessary to avoid overlap (cases where we have fewer
    //chunks, for example, aren't entirely captured by
    //ON DUPLICATE KEY UPDATE
    //queries
    $deleteQueries = array();
    array_push($deleteQueries, "DELETE FROM chain WHERE img_id='".$prevImgID.".jpg';");
    array_push($deleteQueries, "DELETE FROM chunk WHERE img_id='".$prevImgID.".jpg';");
    array_push($deleteQueries, "DELETE FROM mention WHERE img_id='".$prevImgID.".jpg';");
    runUpdateQueries($deleteQueries);
    ...
    if(array_key_exists("chkbx:chainTable", $_POST)) {
        ...
        //Get the box/chain associations from the box/chain table
        $chainAssocBoxes = array();
        foreach($chainTable as $boxID => $chainArr) {
            foreach($chainArr as $chainID => $chkbx) {
                //Add this chain ID to the associated box dict, if we haven't yet
                if(!array_key_exists($chainID, $chainAssocBoxes))
                    $chainAssocBoxes[$chainID] = array();
                array_push($chainAssocBoxes[$chainID], $boxID);
            }
        }

        //For each chain, create an update query based on the associated box IDs
        foreach($chainAssocBoxes as $chainID => $boxArr){
            $assocBoxes = "";
            for($i=0; $i<sizeof($boxArr); $i++){
                if($i > 0)
                    $assocBoxes .= "|";
                $assocBoxes .= $boxArr[$i];
            }

            $query = "INSERT INTO chain (img_id, chain_id, assoc_box_ids) ".
                     "VALUES ('".$prevImgID.".jpg', '".$chainID."', '".
                     $assocBoxes."') ON DUPLICATE KEY UPDATE ".
                     "img_id='".$prevImgID.".jpg', chain_id='".$chainID."', ".
                     "assoc_box_ids='".$assocBoxes."';";
            $debugStr.=$query;
            array_push($boxChainInsertQueryArr, $query);
        }
    }
    runUpdateQueries($boxChainInsertQueryArr);

    //Parse the token assignments in preparation for
    //chunk and mention table updates
    $chunkUpdateQueryArr = array();
    $mentionUpdateQueryArr = array();
    if(array_key_exists("hdn:token", $_POST)) {
        $tokenTable = $_POST["hdn:token"];
        foreach ($tokenTable as $captionIdx => $tokenArr) {
            $chunkTokenDict = array();
            $chunkTypeDict = array();
            $mentionTokenDict = array();
            $mentionChainDict = array();

            foreach ($tokenArr as $tokenIdx => $tokenAssig) {
                $tokenIdx = intval($tokenIdx);
                ...
                foreach ($tokenAssigArr as $assig) {
                    ...
                    if ($assigArr[0] == "chunkIdx") {
                        //intvall returns 0 if the string is null,
                        //which we _quite specifically_ do not want
                        if(!is_null($assigArr[1]) && $assigArr[1] != "null" && $assigArr[1] != "NaN"){
                            $chunkIdx = intval($assigArr[1]);
                        }
                        ...
                        //which we _quite specifically_ do not want
                        if(!is_null($assigArr[1]) && $assigArr[1] != "null" && $assigArr[1] != "NaN"){
                            $entityIdx = intval($assigArr[1]);
                        }
                    }
                }

                //Finally, add this token index to the appropriate
                //position in the chunk / mention ranges, if applicable
                if(!is_null($chunkIdx)){
                    if(!array_key_exists($chunkIdx, $chunkTokenDict)){
                        $chunkTokenDict[$chunkIdx] = array(INF, -INF);
                        $chunkTypeDict[$chunkIdx] = $chunkType;
                    }

                    if($tokenIdx < $chunkTokenDict[$chunkIdx][0]) {
                        $chunkTokenDict[$chunkIdx][0] = $tokenIdx;
                    }
                    if($chunkTokenDict[$chunkIdx][1] < $tokenIdx) {
                        $chunkTokenDict[$chunkIdx][1] = $tokenIdx;
                    }
                }
                if(!is_null($entityIdx)){
                    if(!array_key_exists($entityIdx, $mentionTokenDict)){
                        $mentionTokenDict[$entityIdx] = array(INF, -INF);
                        $mentionChainDict[$entityIdx] = $chainID;
                    }

                    if($tokenIdx < $mentionTokenDict[$entityIdx][0]) {
                        $mentionTokenDict[$entityIdx][0] = $tokenIdx;
                    }
                    if($mentionTokenDict[$entityIdx][1] < $tokenIdx) {
                        $mentionTokenDict[$entityIdx][1] = $tokenIdx;
                    }
                }
            }

            //For each entry in the list of chunk / mention indices, add
            //a new insert/update query to the appropriate table
            foreach($chunkTokenDict as $chunkIdx => $tokenIndices){
                if(!is_nan($chunkIdx)){
                    $query = "INSERT INTO chunk (img_id, caption_idx, chunk_idx, ".
                        "chunk_type, start_token_idx, end_token_idx) ".
                        "VALUES ('".$prevImgID.".jpg', ".$captionIdx.", ".
                        $chunkIdx.", '".$chunkTypeDict[$chunkIdx]."', ".
                        $tokenIndices[0].", ".$tokenIndices[1].") ".
                        "ON DUPLICATE KEY UPDATE chunk_type='".
                        $chunkTypeDict[$chunkIdx]."', start_token_idx=".
                        $tokenIndices[0].", end_token_idx=".
                        $tokenIndices[1].";";
                    array_push($chunkUpdateQueryArr, $query);
                }
            }
            foreach($mentionTokenDict as $mentionIdx => $tokenIndices){
                if(!is_nan($mentionIdx)){
                    $query = "INSERT INTO mention (img_id, caption_idx, mention_idx, ".
                        "chain_id, start_token_idx, end_token_idx, card_str, ".
                        "lexical_type) VALUES ('".$prevImgID.".jpg', ".
                        $captionIdx.", ". $mentionIdx.", '".
                        $mentionChainDict[$mentionIdx]."', ". $tokenIndices[0].
                        ", ".$tokenIndices[1].", NULL, NULL) ".
                        "ON DUPLICATE KEY UPDATE chain_id='".
                        $mentionChainDict[$mentionIdx]."', start_token_idx=".
                        $tokenIndices[0].", end_token_idx=".
                        $tokenIndices[1].", card_str=NULL, lexical_type=NULL;";
                    array_push($mentionUpdateQueryArr, $query);
                }
            }
        }
    }

    //Actually update the chunk and mention tables
    runUpdateQueries($chunkUpdateQueryArr);
    runUpdateQueries($mentionUpdateQueryArr);

    //finally, update the img table to reflect that
    //this image has been reviewed
    $imgQuery = "UPDATE image SET reviewed=1 WHERE img_id='" . $prevImgID . ".jpg';";
    runUpdateQueries(array($imgQuery));
...
<script language="javascript" type="text/javascript">
    ...
    function initTables()
    {
        ...
        var boxCategoryDictStr = "<?php echo getBoxCategoryDictStr();?>";
        ...
        var boxCategoryDictStrArr = boxCategoryDictStr.split("|");
        ...
        //and coords themselves divided by semicolons (;)
        var boxIndex;
        var boxStr;
        var boxID;
        ...
        //the box-category strings should be
        //boxes divided by pipes (|), box IDs divided from categories
        //by hashes (#)
        for(boxIndex=0; boxIndex < boxCategoryDictStrArr.length; boxIndex++){
            boxStr = boxCategoryDictStrArr[boxIndex].split("#");
            boxID = boxStr[0];
            if(boxID != "")
                boxCategoryDict[boxID] = boxStr[1];
        }
    ...
    function updateChainTableElement()
    {
        ...
        chainTableHtm += "<th class=\"centeredTableCell\">Category</th>";
        chainTableHtm += "</tr>";
        ...
        for(boxID in boxCoordDict) {
            ...
            chainTableHtm += "<td class=\"centeredTableCell\" >";
            chainTableHtm += boxCategoryDict[boxID] + "</td>";
            chainTableHtm += "</tr>";
        }
...
<form method="post" action="">
    ...
    <input type="hidden" value="<?php global $debugStr; echo $debugStr;?>"/>
    <table>
        <tr>
            <td>
                <table>
                                    ...
                                    <td>
                                        <table cellpadding="10px">
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="submit" onclick=setEndTimeStamp() id="btn:sub_next" name="btn:sub_next"
